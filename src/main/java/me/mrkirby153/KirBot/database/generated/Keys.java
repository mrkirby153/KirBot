/*
 * This file is generated by jOOQ.
*/
package me.mrkirby153.KirBot.database.generated;


import javax.annotation.Generated;

import me.mrkirby153.KirBot.database.generated.tables.Commands;
import me.mrkirby153.KirBot.database.generated.tables.Feeds;
import me.mrkirby153.KirBot.database.generated.tables.Guild;
import me.mrkirby153.KirBot.database.generated.tables.GuildPermissions;
import me.mrkirby153.KirBot.database.generated.tables.PostedMessages;
import me.mrkirby153.KirBot.database.generated.tables.Users;
import me.mrkirby153.KirBot.database.generated.tables.records.CommandsRecord;
import me.mrkirby153.KirBot.database.generated.tables.records.FeedsRecord;
import me.mrkirby153.KirBot.database.generated.tables.records.GuildPermissionsRecord;
import me.mrkirby153.KirBot.database.generated.tables.records.GuildRecord;
import me.mrkirby153.KirBot.database.generated.tables.records.PostedMessagesRecord;
import me.mrkirby153.KirBot.database.generated.tables.records.UsersRecord;

import org.jooq.ForeignKey;
import org.jooq.Identity;
import org.jooq.UniqueKey;
import org.jooq.impl.AbstractKeys;


/**
 * A class modelling foreign key relationships between tables of the <code>kirbot</code> 
 * schema
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.9.0"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Keys {

    // -------------------------------------------------------------------------
    // IDENTITY definitions
    // -------------------------------------------------------------------------

    public static final Identity<CommandsRecord, Integer> IDENTITY_COMMANDS = Identities0.IDENTITY_COMMANDS;
    public static final Identity<FeedsRecord, Integer> IDENTITY_FEEDS = Identities0.IDENTITY_FEEDS;
    public static final Identity<GuildRecord, Integer> IDENTITY_GUILD = Identities0.IDENTITY_GUILD;
    public static final Identity<GuildPermissionsRecord, Integer> IDENTITY_GUILD_PERMISSIONS = Identities0.IDENTITY_GUILD_PERMISSIONS;
    public static final Identity<PostedMessagesRecord, Integer> IDENTITY_POSTED_MESSAGES = Identities0.IDENTITY_POSTED_MESSAGES;
    public static final Identity<UsersRecord, Integer> IDENTITY_USERS = Identities0.IDENTITY_USERS;

    // -------------------------------------------------------------------------
    // UNIQUE and PRIMARY KEY definitions
    // -------------------------------------------------------------------------

    public static final UniqueKey<CommandsRecord> KEY_COMMANDS_ID_UNIQUE = UniqueKeys0.KEY_COMMANDS_ID_UNIQUE;
    public static final UniqueKey<FeedsRecord> KEY_FEEDS_PRIMARY = UniqueKeys0.KEY_FEEDS_PRIMARY;
    public static final UniqueKey<FeedsRecord> KEY_FEEDS_ID_UNIQUE = UniqueKeys0.KEY_FEEDS_ID_UNIQUE;
    public static final UniqueKey<GuildRecord> KEY_GUILD_PRIMARY = UniqueKeys0.KEY_GUILD_PRIMARY;
    public static final UniqueKey<GuildRecord> KEY_GUILD_ID_UNIQUE = UniqueKeys0.KEY_GUILD_ID_UNIQUE;
    public static final UniqueKey<GuildPermissionsRecord> KEY_GUILD_PERMISSIONS_PRIMARY = UniqueKeys0.KEY_GUILD_PERMISSIONS_PRIMARY;
    public static final UniqueKey<GuildPermissionsRecord> KEY_GUILD_PERMISSIONS_ID_UNIQUE = UniqueKeys0.KEY_GUILD_PERMISSIONS_ID_UNIQUE;
    public static final UniqueKey<PostedMessagesRecord> KEY_POSTED_MESSAGES_PRIMARY = UniqueKeys0.KEY_POSTED_MESSAGES_PRIMARY;
    public static final UniqueKey<UsersRecord> KEY_USERS_PRIMARY = UniqueKeys0.KEY_USERS_PRIMARY;
    public static final UniqueKey<UsersRecord> KEY_USERS_ID_UNIQUE = UniqueKeys0.KEY_USERS_ID_UNIQUE;
    public static final UniqueKey<UsersRecord> KEY_USERS_DISCORD_ID_UNIQUE = UniqueKeys0.KEY_USERS_DISCORD_ID_UNIQUE;

    // -------------------------------------------------------------------------
    // FOREIGN KEY definitions
    // -------------------------------------------------------------------------

    public static final ForeignKey<CommandsRecord, GuildRecord> FK_COMMANDS_GUILD1 = ForeignKeys0.FK_COMMANDS_GUILD1;
    public static final ForeignKey<FeedsRecord, GuildRecord> FK_FEEDS_GUILD1 = ForeignKeys0.FK_FEEDS_GUILD1;
    public static final ForeignKey<GuildPermissionsRecord, GuildRecord> FK_GUILD_PERMISSIONS_GUILD1 = ForeignKeys0.FK_GUILD_PERMISSIONS_GUILD1;
    public static final ForeignKey<GuildPermissionsRecord, UsersRecord> FK_GUILD_PERMISSIONS_USERS1 = ForeignKeys0.FK_GUILD_PERMISSIONS_USERS1;
    public static final ForeignKey<PostedMessagesRecord, FeedsRecord> FK_POSTED_MESSAGES_FEEDS1 = ForeignKeys0.FK_POSTED_MESSAGES_FEEDS1;

    // -------------------------------------------------------------------------
    // [#1459] distribute members to avoid static initialisers > 64kb
    // -------------------------------------------------------------------------

    private static class Identities0 extends AbstractKeys {
        public static Identity<CommandsRecord, Integer> IDENTITY_COMMANDS = createIdentity(Commands.COMMANDS, Commands.COMMANDS.ID);
        public static Identity<FeedsRecord, Integer> IDENTITY_FEEDS = createIdentity(Feeds.FEEDS, Feeds.FEEDS.ID);
        public static Identity<GuildRecord, Integer> IDENTITY_GUILD = createIdentity(Guild.GUILD, Guild.GUILD.ID);
        public static Identity<GuildPermissionsRecord, Integer> IDENTITY_GUILD_PERMISSIONS = createIdentity(GuildPermissions.GUILD_PERMISSIONS, GuildPermissions.GUILD_PERMISSIONS.ID);
        public static Identity<PostedMessagesRecord, Integer> IDENTITY_POSTED_MESSAGES = createIdentity(PostedMessages.POSTED_MESSAGES, PostedMessages.POSTED_MESSAGES.ID);
        public static Identity<UsersRecord, Integer> IDENTITY_USERS = createIdentity(Users.USERS, Users.USERS.ID);
    }

    private static class UniqueKeys0 extends AbstractKeys {
        public static final UniqueKey<CommandsRecord> KEY_COMMANDS_ID_UNIQUE = createUniqueKey(Commands.COMMANDS, "KEY_commands_id_UNIQUE", Commands.COMMANDS.ID);
        public static final UniqueKey<FeedsRecord> KEY_FEEDS_PRIMARY = createUniqueKey(Feeds.FEEDS, "KEY_feeds_PRIMARY", Feeds.FEEDS.ID);
        public static final UniqueKey<FeedsRecord> KEY_FEEDS_ID_UNIQUE = createUniqueKey(Feeds.FEEDS, "KEY_feeds_id_UNIQUE", Feeds.FEEDS.ID);
        public static final UniqueKey<GuildRecord> KEY_GUILD_PRIMARY = createUniqueKey(Guild.GUILD, "KEY_guild_PRIMARY", Guild.GUILD.ID);
        public static final UniqueKey<GuildRecord> KEY_GUILD_ID_UNIQUE = createUniqueKey(Guild.GUILD, "KEY_guild_id_UNIQUE", Guild.GUILD.ID);
        public static final UniqueKey<GuildPermissionsRecord> KEY_GUILD_PERMISSIONS_PRIMARY = createUniqueKey(GuildPermissions.GUILD_PERMISSIONS, "KEY_guild_permissions_PRIMARY", GuildPermissions.GUILD_PERMISSIONS.ID);
        public static final UniqueKey<GuildPermissionsRecord> KEY_GUILD_PERMISSIONS_ID_UNIQUE = createUniqueKey(GuildPermissions.GUILD_PERMISSIONS, "KEY_guild_permissions_id_UNIQUE", GuildPermissions.GUILD_PERMISSIONS.ID);
        public static final UniqueKey<PostedMessagesRecord> KEY_POSTED_MESSAGES_PRIMARY = createUniqueKey(PostedMessages.POSTED_MESSAGES, "KEY_posted_messages_PRIMARY", PostedMessages.POSTED_MESSAGES.ID);
        public static final UniqueKey<UsersRecord> KEY_USERS_PRIMARY = createUniqueKey(Users.USERS, "KEY_users_PRIMARY", Users.USERS.ID);
        public static final UniqueKey<UsersRecord> KEY_USERS_ID_UNIQUE = createUniqueKey(Users.USERS, "KEY_users_id_UNIQUE", Users.USERS.ID);
        public static final UniqueKey<UsersRecord> KEY_USERS_DISCORD_ID_UNIQUE = createUniqueKey(Users.USERS, "KEY_users_discord_id_UNIQUE", Users.USERS.DISCORD_ID);
    }

    private static class ForeignKeys0 extends AbstractKeys {
        public static final ForeignKey<CommandsRecord, GuildRecord> FK_COMMANDS_GUILD1 = createForeignKey(me.mrkirby153.KirBot.database.generated.Keys.KEY_GUILD_PRIMARY, Commands.COMMANDS, "fk_commands_guild1", Commands.COMMANDS.GUILD);
        public static final ForeignKey<FeedsRecord, GuildRecord> FK_FEEDS_GUILD1 = createForeignKey(me.mrkirby153.KirBot.database.generated.Keys.KEY_GUILD_PRIMARY, Feeds.FEEDS, "fk_feeds_guild1", Feeds.FEEDS.GUILD_ID);
        public static final ForeignKey<GuildPermissionsRecord, GuildRecord> FK_GUILD_PERMISSIONS_GUILD1 = createForeignKey(me.mrkirby153.KirBot.database.generated.Keys.KEY_GUILD_PRIMARY, GuildPermissions.GUILD_PERMISSIONS, "fk_guild_permissions_guild1", GuildPermissions.GUILD_PERMISSIONS.GUILD);
        public static final ForeignKey<GuildPermissionsRecord, UsersRecord> FK_GUILD_PERMISSIONS_USERS1 = createForeignKey(me.mrkirby153.KirBot.database.generated.Keys.KEY_USERS_PRIMARY, GuildPermissions.GUILD_PERMISSIONS, "fk_guild_permissions_users1", GuildPermissions.GUILD_PERMISSIONS.USER);
        public static final ForeignKey<PostedMessagesRecord, FeedsRecord> FK_POSTED_MESSAGES_FEEDS1 = createForeignKey(me.mrkirby153.KirBot.database.generated.Keys.KEY_FEEDS_PRIMARY, PostedMessages.POSTED_MESSAGES, "fk_posted_messages_feeds1", PostedMessages.POSTED_MESSAGES.FEED);
    }
}
