...
class CommandRss : CmdExecutor() {

    override fun execute(context: Context, cmdContext: CommandContext) {
        val action = cmdContext.string("action")?.toLowerCase()
        val parameters: List<String> = cmdContext.string("parameters")?.toLowerCase()?.split(" ")?.filter { it.isNotEmpty() } ?: emptyList()
            ...
            RssFeed.get(context.guild).queue { feeds ->
                context.send().embed("RSS Feeds") {
                    description {
                        ...
                        +"\n\n"
                        +"Use `${context.shard.serverSettings[context.guild.id].cmdDiscriminator}rss remove <id>` to delete the feed\n"
                        +"Use `${context.shard.serverSettings[context.guild.id].cmdDiscriminator}rss add <url>` to add a feed\n"
                        +"Use `${context.shard.serverSettings[context.guild.id].cmdDiscriminator}rss refresh <id>` to force a refresh"
                    }
                }.rest().queue()
            }
            return
